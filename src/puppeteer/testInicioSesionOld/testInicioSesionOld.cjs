...
function runEscenario1(url) {
    (async () => {
        ...
        await page.screenshot({ path: './testInicioSesionOld/inicioSesionGhost.png' });
        ...
        await page.screenshot({ path: './testInicioSesionOld/inicioSesionFallidoEnBlanco.png' });
...
function runEscenario2(url) {
    (async () => {
        ...
        // Llenar informacion
        await page.type('input[name="identification"]', 'test.fallido@uniandes.edu.co');
        await page.type('input[name="password"]', 'falla');
        await page.click('button[id="ember12"]');
        await new Promise(r => setTimeout(r, 7000));

        // Captura Estado fallido datos erroneos (Escenario 2)
        await page.screenshot({ path: './testInicioSesionOld/inicioSesionFallidoDatosErroneos.png' });
...
function runEscenario3(user, pass, url) {
    (async () => {
        ...
        await page.screenshot({ path: './testInicioSesionOld/InicioSesionExitoso.png' });
...
function runEscenario4(user, pass, url) {
    (async () => {
        ...
        await page.screenshot({ path: './testInicioSesionOld/InicioSesionCerradoSesion.png' });
        ...
        await page.screenshot({ path: './testInicioSesionOld/CerradoSesion.png' });
